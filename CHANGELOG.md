...
All notable changes to Athena will be documented in this file. Release versions follow the format [Major].[Minor].[Patch], where Major indicates breaking changes, Minor denotes new features, and Patch represents bug fixes. This project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).
...
- Changed Server code to use named paramaters in all constructor and function calls for improved code standardization, readability, maintainability and reduced risk of errors.
...
- Changed Server `AuthController::refresh` function so that the `refreshToken` in the session is rotated instead of revolking the entire session and building a new one.
- Changed Database `ai_message.timestamp` coulmn to use the more standard `created_at` name.
- Changed Database `audit_log.timestamp` coulmn to use the more standard `created_at` name.
- Changed Database `system_log.timestamp` coulmn to use the more standard `created_at` name.
...
- Fixed Server `delete` and `update` functions to now explicitly verify user ownership of objects before comitting changes.
